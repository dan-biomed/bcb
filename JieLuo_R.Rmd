...
##same output, comma is to be clear
#snp_cut_col<-snp[c(1,3,4)]
#snp_cut_col<-snp[,c(1,3,4)]
...
ggplot(tidied_maize)+geom_bar(aes(x=Chromosome))


#then if homozygous, then assign "homozygous" to the column homohetero
tidied_maize<-within(tidied_maize, {
  homohetero<-NA
...
tidied_maize<-as.data.frame(lapply(tidied_maize, function(x) {
  gsub("\\?\\?", NA,x)
}))

tidied_maize<-tidied_maize %>% arrange(Sample_ID)


ggplot(tidied_maize,aes(x=Sample_ID, fill=homohetero))+
  geom_bar(position="fill")

ggplot(tidied_maize)+geom_bar(aes(x=homohetero))
